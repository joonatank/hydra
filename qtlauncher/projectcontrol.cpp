 ...
 *	2010-12
 *	Teplaced spaces with tabs
 *	Fixed destructor segfaulting because deleting _treePrarent multiple times
...
void ProjControl::createProj()
{
	...
	QString projName = QString::fromStdString(_projSet->getCasePtr()->getName());
	ProjPage* projPage = new ProjPage( _projSet, _base, _treeParent, projName );

	connect( projPage, SIGNAL(launch_clicked(QString const &)),
			 this, SLOT(launch_clicked(QString const &)));
