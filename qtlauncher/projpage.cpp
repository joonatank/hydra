...
ProjPage::ProjPage( vl::ProjSettingsRefPtr projSet, QWidget* base,
					QTreeWidgetItem* treeParent, QString const & name ) :
        ui( new Ui::ProjPage() ),
        _projSet( projSet )
{
	std::cout << "ProjPage constructor: name " << name.toStdString()
		<< "." << std::endl;
...
{
	std::cout << "ProjPage update: name  " << ui->name->text().toStdString()
		<< std::endl;
	...
	{
		vl::ProjSettings::Scene const *s = _case->getScenePtr(i);
	...
	{
		vl::ProjSettings::ActionMap const *m = _case->getMapPtr(u);
	...
	{
		vl::ProjSettings::Script const *m = _case->getScriptPtr(a);
		QFileInfo file( QString::fromStdString( m->getFile() ) );
...
{
	std::cout << "ProjPage changePage: item name  "
		<< item->text(0).toStdString() << std::endl;
                         ...
                         QString const &attachto_point )
{
	std::cout << "ProjPage addScene: page name  "
		<< ui->name->text().toStdString() << " : scene name "
...
{
	std::cout << "ProjPage addScript: page name  "
		<< ui->name->text().toStdString() << " : script file "
		<< file.filePath().toStdString() << std::endl;
...
{
	std::cout << "ProjPage addScene: page name  "
		<< ui->name->text().toStdString() << " : new name "
		<< name.toStdString() << std::endl;
...
void ProjPage::map_toggled(QWidget* but)
{
	...
	vl::ProjSettings::ActionMap *map = _case->getMapPtr( box->text().toStdString() );
	map->mapOnOff( box->isChecked() );
}
...
void ProjPage::script_toggled(QWidget* but)
{
	...
	vl::ProjSettings::Script *script = _case->getScriptPtr( box->text().toStdString() );
	script->scriptOnOff( box->isChecked() );
}
